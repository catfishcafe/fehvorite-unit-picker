...
(function (root, factory) {
  ...
  function PickerUI(picker, options) {
    ...
    this.messages = $.extend(
      {
        reset: "Reset",
        mustSelect:
          "You must select something first! If you're indifferent, press Pass.",
        orderedAll: "You have ordered every available item!",
        noItems:
          "There are no items that fit your criteria! Set some different options and try again.",
        resetWarning:
          "Are you sure you wish to reset your state? All your found favorites and current progress will be lost.",
      },
    ...
    if (this.elem.sharedListContinue) {
      this.elem.sharedListContinue.on("click", function (e) {
        e.preventDefault();
        self.picker.resetToFavorites(
          $.map(self.picker.getSharedFavorites(), function (item) {
            return item.id;
          })
        );
    ...
    if (type === "checkbox" || type === "radio") {
      if ($elem.length === 1) {
        ...
        $elem.each(function () {
          ...
          $(this).prop(
            "checked",
            $.isArray(value) ? value.indexOf(val) !== -1 : value === val
          );
        });
      }
    } else {
  ...
  PickerUI.prototype.displayEmpty = function () {
     ...
     */
    var item = this.wrapItem(
      this.picker.hasItems()
        ? this.messages.orderedAll + " "
        : this.messages.noItems
    ).addClass("notice");
  ...
  PickerUI.prototype.displayBatch = function () {
    ...
    $.each(batch, function () {
      self.elem.evaluating.append(
        self.getItemElem(this, self.picker.getSettings())
      );
    });
  ...
  PickerUI.prototype.updateFavorites = function () {
    ...
    $.each(favorites, function () {
      self.elem.favorites.append(
        self.getItemElem(this, self.picker.getSettings())
      );
    });
    if (
      this.elem.shortcodeLink &&
      this.picker.options.favoritesQueryParam &&
      this.picker.options.shortcodeLength
    ) {
      this.elem.shortcodeLink
        .attr("href", this.picker.getShortcodeLink())
        .toggle(favorites.length > 0);
    }
  };
  ...
  PickerUI.prototype.getItemElem = function (item, settings) {
    ...
    if (this.options.getItemElem) {
      return $(this.options.getItemElem(item, settings))
        .addClass("item")
        .data("item", item.id);
    }
  ...
  PickerUI.prototype.makeResetButton = function (text) {
    ...
    var self = this;
    return $(
      '<button type="button">' + (text || this.messages.reset) + "</button>"
    ).on("click", function () {
      self.reset();
    });
  };
