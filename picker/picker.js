...
(function (root, factory) {
  ...
  PickerState.prototype.setRoundSize = function () {
     ...
     */
    this.roundSize = Math.max(
      0,
      this.items.length -
        this.arrays.favorites.length -
        this.arrays.eliminated.length -
        1
    );
  };
  ...
  Picker.prototype.initialize = function () {
    ...
    for (i = 0; i < this.options.items.length; i++) {
      ...
      if (
        ...
        (!this.options.items[i].shortcode ||
          this.options.items[i].shortcode.length !==
            this.options.shortcodeLength)
      ) {
        console.error(
    ...
    var pickerStateOptions = {
      ...
      shouldIncludeItem:
        this.options.shouldIncludeItem &&
        function (identifier, settings) {
          return self.options.shouldIncludeItem(
            self.itemMap[identifier],
            settings
          );
        },
